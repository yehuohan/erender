...
pub fn look_at(eye: &Vec3, center: &Vec3, up: &Vec3) -> Mat4 {
    ...
    let v = {
        ...
        let v = Mat4::from(r.to_vec4(0.0), u.to_vec4(0.0), d.to_vec4(0.0), Vec4::from(0.0, 0.0, 0.0, 1.0));
        v
    };
...
mod tests {
    ...
    fn interpolation() {
        ...
        let o = lerp(&a, &b, 0.5);
        println!("lerp: \n{}", o);
        assert!(approx_eq!(Vec3, o, Vec3::from(0.5, 0.5, 2.5), epsilon = 0.000001));
        ...
        println!("interpolate: \n{}", o);
        assert!(approx_eq!(Vec3, o, Vec3::fill(1.0) / 3.0, epsilon = 0.000001));
    }
    ...
    fn reflect_refract() {
        ...
        let o = reflect(&i, &n);
        println!("reflect: {}", o);
        assert!(approx_eq!(Vec3, o, Vec3::from(-1.0, -1.0, 1.0), epsilon = 0.000001));
    ...
    fn intersection() {
        ...
        if let Some(t) = intersect_sphere(&center, radius, &ori, &dir) {
            ...
            assert!(approx_eq!(Tyf, t, 4.108, epsilon = 0.001));
        } else {
        ...
        if let Some((t, bc)) = intersect_triangle(&abc, &ori, &dir) {
            ...
            assert!(approx_eq!(Tyf, t, 0.577, epsilon = 0.001));
            assert!(approx_eq!(Vec3, bc, Vec3::fill(1.0) / 3.0, epsilon = 0.000001));
        } else {
